...
export const createMessageSlice: StateCreator<ChatStore, [], [], MessageActions> = (set, get) => ({
  ...
  stopStreaming: (sessionId, reason) => {
    ...
    set((state) => ({
      sessions: updateSession(state.sessions, sessionId, (current) => ({
        ...
        ...(reason ? {
          messages: [
            ...current.messages,
            { role: "assistant", content: `✗ ${reason}`, timestamp: Date.now() },
          ],
        } : {}),
      })),
    }));
  ...
  upsertRuntimeWriteProgress: (sessionId, bookId, write) => {
    /**
     * 将后端 runtime-status 恢复为聊天中的工具执行块，确保刷新后仍可见进度。
     * @author zjh
     * @date 2026-05-08
     */
    set((state) => ({
      sessions: updateSession(state.sessions, sessionId, (runtime) => {
        const toolId = `runtime-write-${bookId}`;
        const stagesBase = [
          { key: "planning", label: "准备写作材料" },
          { key: "writing", label: "撰写章节草稿" },
          { key: "auditing", label: "审查章节内容" },
          { key: "revising", label: "修订章节文本" },
          { key: "persisting", label: "保存章节与状态" },
        ] as const;
        const activeIdx = stagesBase.findIndex((s) => s.key === (write.stage ?? "planning"));
        const stages = stagesBase.map((s, idx) => {
          if (write.status === "failed") {
            return {
              label: s.label,
              status: idx < Math.max(0, activeIdx) ? "completed" as const : "pending" as const,
              progress: idx === activeIdx && typeof write.progress === "number"
                ? {
                    status: `${Math.round(write.progress)}%`,
                    elapsedMs: 0,
                    totalChars: 0,
                    chineseChars: 0,
                  }
          ...
          if (write.status === "completed") {
            return { label: s.label, status: "completed" as const };
          }
          if (write.status === "running") {
            if (idx < activeIdx) return { label: s.label, status: "completed" as const };
            if (idx === activeIdx) {
              return {
                label: s.label,
                status: "active" as const,
                progress: {
                  status: `${Math.round(write.progress ?? 0)}%`,
                  elapsedMs: 0,
                  totalChars: 0,
                  chineseChars: 0,
                },
        ...
        const execStatus = write.status === "failed"
          ? "error"
        ...
        const execution = {
          ...
          agent: "writer",
          label: "写作",
          status: execStatus as "running" | "processing" | "completed" | "error",
          args: { agent: "writer", bookId },
          stages,
          ...(write.error ? { error: write.error } : {}),
          startedAt: Date.now(),
          ...(write.status === "completed" || write.status === "failed" ? { completedAt: Date.now() } : {}),
        };

        const messages = [...runtime.messages];
        const last = messages[messages.length - 1];
        if (last?.role === "assistant" && last.parts?.some((p) => p.type === "tool")) {
          const parts = last.parts.map((p) => {
            if (p.type !== "tool") return p;
            const isExactRuntimeTool = p.execution.id === toolId;
            const isWriterTool = p.execution.tool === "sub_agent" && p.execution.agent === "writer";
            if (!isExactRuntimeTool && !isWriterTool) return p;
            return {
              type: "tool" as const,
              execution: {
                ...execution,
                // 复用已有卡片，避免刷新恢复时出现重复“写作过程卡片”
                id: p.execution.id,
          ...
          messages[messages.length - 1] = { ...last, ...flat, parts };
          return { messages };
        }

        if (write.status === "running" || write.status === "failed") {
          const parts = [{ type: "tool" as const, execution }];
          const flat = deriveFlat(parts);
          messages.push({
            role: "assistant",
            ...
            ...(flat.thinkingStreaming ? { thinkingStreaming: flat.thinkingStreaming } : {}),
  ...
  sendMessage: async (sessionId, text, activeBookId) => {
    ...
    const paperPipelineShortcut =
      activeBookId
      && /^(开始写论文|帮我写论文|请帮我写论文|生成论文|生成整篇论文|跑流水线|跑一下流水线|一键生成|一键生成论文|启动流水线|启动论文流水线)[。！？\s]*$/i.test(
        trimmed,
      );

    if (paperPipelineShortcut) {
      get().addUserMessage(sessionId, trimmed);
      try {
        await postApi(`/papers/${activeBookId}/pipeline/start`, {});
        set((state) => ({
          sessions: updateSession(state.sessions, sessionId, (session) => ({
            messages: [
              ...session.messages,
              {
                role: "assistant",
                content:
                  "已启动全量论文流水线。请在左侧打开该论文下的「论文生成」查看阶段进度，完成后下载 Word。",
                timestamp: Date.now(),
              },
            ],
          })),
        }));
        window.location.hash = `#/paper/${encodeURIComponent(activeBookId)}/generate`;
      } catch (err) {
        get().addErrorMessage(sessionId, err instanceof Error ? err.message : String(err));
      }
      return;
    ...
    const streamIdleTimer = window.setInterval(() => {
      const runtime = get().sessions[sessionId];
      if (!runtime?.isStreaming) {
        window.clearInterval(streamIdleTimer);
        return;
      }
      if (Date.now() - lastActivityAt < STREAM_IDLE_TIMEOUT_MS) return;
      streamEs.close();
      get().addErrorMessage(sessionId, "连接超时：长时间未收到进度，请重试。");
      set((state) => ({
        sessions: updateSession(state.sessions, sessionId, (current) => ({
          isStreaming: false,
          stream: current.stream === streamEs ? null : current.stream,
        })),
      }));
      window.clearInterval(streamIdleTimer);
    }, 5000);
    ...
    attachSessionStreamListeners({ sessionId, streamTs, streamEs, set, get, onActivity: markActivity });
