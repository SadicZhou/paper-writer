...
export interface BookActivity {
  ...
  readonly stage: "idle" | "planning" | "writing" | "auditing" | "revising" | "persisting";
...
export function deriveBookActivity(messages: ReadonlyArray<SSEMessage>, bookId: string): BookActivity {
  ...
  let stage: BookActivity["stage"] = "idle";
  let stageProgress = 0;
  ...
  for (const message of messages) {
    ...
    switch (message.event) {
      case "write:start":
        ...
        break;
      case "write:stage": {
        const payload = message.data as { stage?: unknown; progress?: unknown };
        if (
          payload?.stage === "planning"
          || payload?.stage === "writing"
          || payload?.stage === "auditing"
          || payload?.stage === "revising"
          || payload?.stage === "persisting"
        ) {
          stage = payload.stage;
        }
        if (typeof payload?.progress === "number") {
          stageProgress = Math.max(stageProgress, Math.min(100, payload.progress));
        }
        break;
      }
      case "write:complete":
  ...
  return { writing, drafting, lastError, stage, stageProgress };
}
