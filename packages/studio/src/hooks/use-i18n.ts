...
const strings = {
  ...
  "import.canonModeBook": { zh: "项目内书籍", en: "Book in this project" },
  "import.canonModeLocal": { zh: "本地文件 / 粘贴文本", en: "Local file / pasted text" },
  "import.chooseCanonFile": { zh: "选择本地文件 (.txt / .md)", en: "Choose local file (.txt / .md)" },
  "import.canonPastePlaceholder": { zh: "或直接粘贴母本正文、设定汇编…", en: "Or paste canon text / setting notes…" },
  "import.canonLocalHint": { zh: "内容与「导入章节」类似：从磁盘选文件或粘贴；会生成 story/parent_canon.md。", en: "Like chapter import: pick a file or paste. Writes story/parent_canon.md." },
  "import.canonSourceLabel": { zh: "母本来源备注（写入 meta，选填）", en: "Source label for meta (optional)" },
  "import.canonTruncateWarn": {
    zh: "超过服务端单次上限：超出部分不会被发送给模型（可删减或分批）。",
    en: "Exceeds single-request limit; excess is not sent to the model.",
  },
  "import.large.chunkSize.label": { zh: "分块大小（chunk size）", en: "Chunk size" },
  "import.large.chunkSize.hint": { zh: "每块送入模型的字符数。越大上下文越完整，但更耗时/耗 token。", en: "Chars per chunk sent to model. Larger keeps more context but costs more time/tokens." },
  "import.large.overlap.label": { zh: "重叠大小（overlap）", en: "Overlap" },
  "import.large.overlap.hint": { zh: "相邻块重叠字符，减少跨块信息断裂。越大去重压力越高。", en: "Overlapped chars between adjacent chunks to reduce boundary loss; larger overlap increases dedupe pressure." },
  "import.large.shardSize.label": { zh: "分片规模（shard size）", en: "Shard size" },
  "import.large.shardSize.hint": { zh: "多少个 chunk 合并成一个中间汇总。越大 reduce 次数越少，但单次合并更重。", en: "How many chunks merged per shard. Larger reduces merge rounds but makes each merge heavier." },
  "import.large.concurrency.label": { zh: "并发数（concurrency）", en: "Concurrency" },
  "import.large.concurrency.hint": { zh: "Map 阶段并行提炼的 chunk 数。越大越快，但更容易触发限流。", en: "Parallel chunk extraction in map stage. Higher is faster but may hit rate limits." },
  ...
  "doctor.inkosJson": { zh: "项目配置文件", en: "Project config file" },
  "doctor.projectEnv": { zh: "项目 .env 文件", en: "Project .env file" },
  "doctor.globalEnv": { zh: "全局环境配置", en: "Global environment config" },
