...
describe("deriveBookActivity", () => {
  ...
  it("tracks write stage events for audit and revise visibility", () => {
    const messages: ReadonlyArray<SSEMessage> = [
      msg("write:start", { bookId: "alpha" }, 1),
      msg("write:stage", { bookId: "alpha", stage: "writing", progress: 45 }, 2),
      msg("write:stage", { bookId: "alpha", stage: "auditing", progress: 70 }, 3),
    ];
    expect(deriveBookActivity(messages, "alpha")).toMatchObject({
      writing: true,
      stage: "auditing",
