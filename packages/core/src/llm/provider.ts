...
export function createLLMClient(config: LLMConfig): LLMClient {
  ...
  const piApi = resolvePiApi(serviceName, config.apiFormat, (inkosProvider?.api ?? preset?.api) as PiApi, baseUrl, provider) as PiApi;
  ...
  /** DeepSeek 等走 Anthropic Messages 兼容网关时，providerFamily 仍为 openai，需按 URL 纠正。 */
  else if (baseUrl.includes("/anthropic")) piProvider = "anthropic";
...
function resolvePiApi(
  ...
  // When targeting an Anthropic-compatible endpoint (e.g. DeepSeek's /anthropic),
  // use the Anthropic Messages API format regardless of the service preset's
  // native protocol, so the pi-ai SDK formats the request body correctly.
...
async function chatCompletionViaPiAi(
  ...
  // Disable thinking for DeepSeek models to prevent output tokens
  // being consumed by reasoning blocks instead of text content.
  // pi-ai's buildParams only applies thinking settings when model.reasoning is true,
  // so we must override the piModel.reasoning flag for DeepSeek models.
  if (client.service === "deepseek" || piModel.id.includes("deepseek")) {
    piModel.reasoning = true;
    streamOpts.thinkingEnabled = false;
  }
